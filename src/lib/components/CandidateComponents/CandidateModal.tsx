      ...
      >
        ...
        >
          ...
          <div className="modal-body">
            ...
            {activeTab === "history" && (
              <CandidateApplicationHistory candidate={candidate} interviews={interviews} />
            )}
          </div>
        </div>
      </div>
...
function CandidateCVAnalysis({
  ...
  const getActiveInterviews = (interviews: any[]) => {
    return interviews?.filter(
      (interview: any) => !["Dropped", "Cancelled"].includes(interview.applicationStatus)
    );
  };
  ...
  async function regenerateCV() {
    const interviewsToProcess = interviews.filter(
      (interview: any) =>
        !["Dropped", "Cancelled"].includes(interview.applicationStatus) &&
    ...
    try {
      ...
      for (const interview of interviewsToProcess) {
        ...
        updatedInterviews = updatedInterviews.map((i: any) =>
                  ...
                  {({ loading }) =>
                    loading ? (
                      ...
                      <>
                        ...
                        ></i>
                        <span style={{ color: cvData?.length > 0 ? "#414651" : "#1010104d" }}>
                          Download CV
                        </span>
                      </>
                    )
          ...
          <div className="layered-card-content">
            {regenerateLoading ? (
              <LoadingAnimation
                ...
                subtext="JIA is analyzing the CV..."
              />
            ) : getActiveInterviews(interviews)?.length > 0 ? (
              getActiveInterviews(interviews).map((interview: any) => (
      ...
      {isLoading ? (
            ...
            {[...Array(6)].map((_, index) => (
              <div key={index} className="layered-card-middle">
                <div
                  ...
                  style={{ width: "30%", height: "20px", marginBottom: 8 }}
                ></div>
        ...
        >
          <div style={{ width: "60%", display: "flex", flexDirection: "column", gap: 8 }}>
            {/* Current Experience */}
            <div className="layered-card-middle">
              <span style={{ fontSize: 16, color: "#181D27", fontWeight: 700, marginLeft: 16 }}>
                Experience
              </span>
              {cvData?.find((section) => section.name === "Experience") && (
                <div className="layered-card-content">
                  <Markdown>
                    {cvData
                      ?.find((section: any) => section.name === "Experience")
                      ?.content.split("**Experience**")?.[1]
                      ?.trim()}
                  </Markdown>
                </div>
              )}
            ...
            <div className="layered-card-middle">
              ...
              {cvData?.find((section) => section.name === "Education") && (
                <div className="layered-card-content">
                  <Markdown>
                    {cvData
                      ?.find((section: any) => section.name === "Education")
                      ?.content.split("**Education**")?.[1]
                      ?.trim()}
                  </Markdown>
                </div>
              )}
            ...
            <div className="layered-card-middle">
              ...
              {cvData?.find((section) => section.name === "Skills") && (
                <div className="layered-card-content">
                  <Markdown>
                    {cvData
                      ?.find((section: any) => section.name === "Skills")
                      ?.content.split("**Skills**")?.[1]
                      ?.trim()}
                  </Markdown>
                </div>
              )}
            ...
            <div className="layered-card-middle">
              ...
              {cvData?.find((section) => section.name === "Certifications") && (
                <div className="layered-card-content">
                  <Markdown>
                    {cvData
                      ?.find((section: any) => section.name === "Certifications")
                      ?.content.split("**Certifications**")?.[1]
                      ?.trim()}
                  </Markdown>
                </div>
              )}
            ...
            <div className="layered-card-middle">
              ...
              {cvData?.find((section) => section.name === "Projects") && (
                <div className="layered-card-content">
                  <Markdown>
                    {cvData
                      ?.find((section: any) => section.name === "Projects")
                      ?.content.split("**Projects**")?.[1]
                      ?.trim()}
                  </Markdown>
                </div>
              )}
            ...
            <div className="layered-card-middle">
              ...
              {cvData?.find((section) => section.name === "Awards") && (
                <div className="layered-card-content">
                  <Markdown>
                    {cvData
                      ?.find((section: any) => section.name === "Awards")
                      ?.content.split("**Awards**")?.[1]
                      ?.trim()}
                  </Markdown>
                </div>
              )}
          ...
          >
            ...
            <div className="layered-card-middle">
              ...
              {cvData?.find((section) => section.name === "Contact Info") && (
                <div className="layered-card-content">
                  <Markdown>
                    {cvData
                      ?.find((section: any) => section.name === "Contact Info")
                      ?.content.split("**Contact Info**")?.[1]
                      ?.trim()}
                  </Markdown>
                </div>
              )}
...
function CandidateApplicationHistory({
  candidate,
  interviews,
}: {
  candidate: any;
  ...
  const filteredApplications = React.useMemo(() => {
    ...
    if (filterAssessment !== "All Assessments") {
      filtered = filtered.filter((interview: any) =>
        interview.assessment.includes(filterAssessment)
      );
    }
    return filtered;
      ...
      for (const interview of interviews) {
        if (
          ...
          if (interview.status === "For Interview" || interview.status === "For AI Interview") {
            newApplications.push({
              ...interview,
              stage: "Pending AI Interview",
              assessment: `CV: ${interview.cvStatus || "N/A"}`,
            });
            continue;
          }
          ...
          newApplications.push({
            ...
            assessment: `Interview: ${interview.jobFit || "N/A"}`,
          });
          continue;
        }
        ...
        if (interview.currentStep === "Applied") {
          newApplications.push({
            ...interview,
            stage: "Applied",
            assessment: `CV: ${interview.status || "N/A"}`,
          });
          continue;
        }
      }
      ...
      >
        ...
        >
          <div style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: 8 }}>
            ...
            >
              <span style={{ fontSize: 12, color: "#181D27", fontWeight: 700 }}>
                {applications?.length}
              </span>
            </div>
          </div>
        ...
        <div className="table-responsive">
          <table className="table align-items-center table-flush">
            <thead>
              <tr>
                <th scope="col" style={{ textTransform: "none", fontWeight: 550 }}>
                  Position Applied
                </th>
            ...
            <tbody>
              {filteredApplications?.map((interview: any, index: number) => (
                ...
                >
                  ...
                  <td>
                    <CareerFit
                      fit={interview.assessment}
                      assessment={
                        interview.currentStep === "CV Screening" ||
                        interview.stage === "Pending AI Interview"
                          ? interview.cvScreeningReason
                          : extractInterviewAssessment(interview?.summary)
                      }
                    />
                  </td>
                  <td>
                    <ApplicantStatusBadge status={interview.applicationStatus || "Ongoing"} />
                  </td>
                  <td>{interview.stage}</td>
                  <td>
                    {interview.applicationMetadata?.updatedAt
                      ? new Date(interview.applicationMetadata.updatedAt).toLocaleDateString(
                          "en-US",
