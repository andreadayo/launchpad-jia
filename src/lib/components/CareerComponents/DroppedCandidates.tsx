    ...
    if (filterAssessment !== "All Assessments") {
      filtered = filtered.filter((candidate) =>
        candidate.currentStep === "CV Screening"
          ? candidate?.cvStatus === filterAssessment
          : candidate?.jobFit === filterAssessment
      );
    }
    ...
    if (filterDroppedBy !== "All") {
      filtered = filtered.filter((candidate) =>
        candidate.applicationStatus === "Cancelled"
          ? candidate.name === filterDroppedBy
          : (candidate.applicationMetadata?.updatedBy?.name || "Jia") === filterDroppedBy
      );
    }
        ...
        >
          <div className="modal-header" style={{ padding: "1.25rem 1.25rem 0 1.25rem" }}>
            <div>
              ...
              <p style={{ fontSize: 14, fontWeight: 500, color: "#717680" }}>{stage}</p>
            </div>
            <button
              style={{ background: "none", border: "none", cursor: "pointer" }}
              onClick={() => handleDroppedCandidatesOpen(false)}
            >
          ...
          <div className="modal-body" style={{ padding: "0 1.25rem 1.25rem 1.25rem" }}>
            ...
            >
              <div style={{ display: "flex", gap: 10 }}>
                ...
                />
                <CustomDropdown
                  value={filterDroppedBy}
                  setValue={setFilterDroppedBy}
                  options={droppedByOptions}
                  icon="la-filter"
                  valuePrefix="Dropped By:"
                />
              </div>
            ...
            <div className="layered-card-outer" style={{ height: "100%", overflowY: "auto" }}>
              <div
                ...
                style={{ height: "100%", background: "#FFFFFF", borderRadius: "20px" }}
              >
                <table
                      ...
                      filteredCandidates.map((candidate: any) => (
                        <tr key={candidate._id} style={{ cursor: "default" }}>
                            ...
                            <CareerFit
                              fit={
                                ...
                                candidate?.stage === "Pending AI Interview"
                                  ...
                                  : `Interview: ${candidate?.jobFit || "N/A"}`
                              }
                              assessment={
                                candidate.currentStep === "CV Screening" ||
                                candidate?.stage === "Pending AI Interview"
                          ...
                          <td>
                            {candidate.applicationStatus === "Cancelled"
                              ? candidate.name
                              : candidate.applicationMetadata?.updatedBy?.name || "Jia"}
                          </td>
                              ...
                              {menuOpen && candidate._id === selectedCandidate?._id && (
                                ...
                                >
                                  ...
                                  <div
                                    ...
                                    onClick={() => handleSelectMenuOption()}
                                  >
                                    <i
