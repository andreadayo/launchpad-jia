        ...
        <div className="layered-card-outer" key={idx}>
          ...
          >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                ...
                >
                  <span style={{ fontSize: 12, color: "#363F72", fontWeight: 700 }}>
                    {timelineStages[stage].candidates?.length || 0}
                  </span>
                </div>
              </div>
            ...
            <div style={{ display: "flex", justifyContent: "center", width: "100%" }}>
              <button
                className="dropped-candidates-btn"
                onClick={() => handleDroppedCandidatesOpen(stage)}
              >
                <div>
                  <i className="la la-user-times" style={{ fontSize: 20, color: "#787486" }}></i>{" "}
                  {timelineStages[stage].droppedCandidates?.length || 0} Dropped Candidates
                </div>
              </button>
            </div>
            {timelineStages[stage]?.candidates?.length > 0 ? (
              timelineStages[stage].candidates.map((c: any, idx: number) => (
                <CandidateCard
                  key={idx}
                  candidate={c}
                  stage={stage}
