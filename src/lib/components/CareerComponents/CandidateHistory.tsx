...
export default function CandidateHistory({ candidate, setShowCandidateHistory }: any) {
    ...
    const fetchInterviewLogs = async () => {
      try {
        ...
        const response = await axios.get(
          `/api/get-interview-history?interviewUID=${candidate._id}`
        );
  ...
  const getLogDescription = (log: any) => {
    ...
    if (log.action === "Cancelled") {
      const reason =
        candidate.selectedReason === "Others" ? candidate.cancelReason : candidate.selectedReason;
      return `${log.action} the application at ${log.fromStage} stage for this following reason: ${reason || "No reason provided"}`;
    }
    return (
      `${log.action} ${candidate.name} from ${log.fromStage}` +
      (log.toStage ? ` to ${log.toStage}` : "")
    );
  };
  return (
    <div
        ...
        >
          <div className="modal-header">
            ...
            <button
              ...
              onClick={() => setShowCandidateHistory(false)}
            >
            ...
            {isLoading ? (
              ...
              <div className="table-responsive" style={{ height: "100%" }}>
                <table
                  ...
                  style={{ border: "1px solid #E9EAEB", borderRadius: 8 }}
                >
