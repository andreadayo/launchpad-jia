  ...
  return (
    ...
    >
      <div className="candidate-card-section" style={{ justifyContent: "space-between" }}>
        <CareerFit
          fit={
            currentStep === "CV Screening" || stage === "Pending AI Interview"
              ? `CV: ${cvStatus || "N/A"}`
              : `Interview: ${jobFit || "N/A"}`
          }
          assessment={
            currentStep === "CV Screening" || stage === "Pending AI Interview"
              ? cvScreeningReason
        ...
        <div className="dropdown">
          ...
          {menuOpen && (
            <div
              className={`dropdown-menu w-100 mt-1 org-dropdown-anim${menuOpen ? " show" : ""}`}
              style={{
                padding: "10px 0px",
              }}
            >
      ...
      <div className="candidate-card-section" style={{ justifyContent: "space-between" }}>
        ...
        <div style={{ fontSize: 12, color: "#787486" }}>
          <img
            src={candidate.applicationMetadata?.updatedBy?.image || "/jia-avatar.png"}
            alt="Jia Avatar"
            width={20}
            height={20}
            style={{ marginRight: 4, borderRadius: "50%" }}
          />
          {candidate.applicationMetadata?.action && candidate.applicationMetadata?.updatedBy
            ? `${candidate.applicationMetadata?.action} by ${candidate.applicationMetadata?.updatedBy?.name}`
            : "Assessed by JIA"}
        </div>
      </div>
    </div>
