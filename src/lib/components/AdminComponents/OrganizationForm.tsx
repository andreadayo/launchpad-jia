...
export default function OrganizationForm({
  ...
  const [companyRegistrationFile, setCompanyRegistrationFile] = useState<File | string | null>(
    organization?.documents?.find((document: any) => document.name === "Company/SEC Registration")
      ?.filename || ""
  );
  const [businessPermitFile, setBusinessPermitFile] = useState<File | string | null>(
    organization?.documents?.find((document: any) => document.name === "Business Permit")
      ?.filename || ""
  );
  ...
  const [members, setMembers] = useState<{ email: string; role: string; error?: string }[]>(
        ...
        if (response.status === 200) {
          const imageUrl =
            image && typeof image === "object"
              ? await uploadFile(
                  image as File,
                  `organization/profile-image/${response.data.orgID}.${(image as File).type.split("/")[1]}`
                )
              : null;
          const coverImageUrl =
            coverImage && typeof coverImage === "object"
              ? await uploadFile(
                  coverImage as File,
                  `organization/cover-image/${response.data.orgID}.${(coverImage as File).type.split("/")[1]}`
                )
              : null;
          const businessPermitUrl =
            businessPermitFile && typeof businessPermitFile === "object"
              ? await uploadFile(
                  ...
                  `organization/documents/business-permit/${response.data.orgID}.${(businessPermitFile as File).type.split("/")[1]}`
                )
              : null;
          const companyRegistrationUrl =
            companyRegistrationFile && typeof companyRegistrationFile === "object"
              ? await uploadFile(
                  companyRegistrationFile as File,
                  `organization/documents/company-registration/${response.data.orgID}.${(companyRegistrationFile as File).type.split("/")[1]}`
                )
              : null;
          const documents = [];
        ...
        if (image) {
          update.image =
            typeof image === "object"
              ? await uploadFile(
                  image as File,
                  `organization/profile-image/${organization._id}.${(image as File).type.split("/")[1]}`
                )
              : image;
        }
        if (coverImage) {
          update.coverImage =
            typeof coverImage === "object"
              ? await uploadFile(
                  coverImage as File,
                  `organization/cover-image/${organization._id}.${(coverImage as File).type.split("/")[1]}`
                )
              : coverImage;
        }
          ...
          if (typeof businessPermitFile === "object") {
            document = {
              ...
              filePath: await uploadFile(
                businessPermitFile as File,
                `organization/documents/business-permit/${organization._id}.${(businessPermitFile as File).type.split("/")[1]}`
              ),
              fileType: (businessPermitFile as File).type.split("/")[1],
            };
          }
        ...
        if (companyRegistrationFile) {
          ...
          if (typeof companyRegistrationFile === "object") {
            document = {
              ...
              filePath: await uploadFile(
                companyRegistrationFile as File,
                `organization/documents/company-registration/${organization._id}.${(companyRegistrationFile as File).type.split("/")[1]}`
              ),
              fileType: (companyRegistrationFile as File).type.split("/")[1],
            };
          }
    ...
    <div className="col">
      {formType === "add" ? (
        ...
        >
          ...
          <button
            ...
            style={{
              width: "fit-content",
              background: !isFormValid() || isSavingOrganization ? "#D5D7DA" : "black",
              color: "#fff",
              border: "1px solid #E9EAEB",
              padding: "8px 16px",
              borderRadius: "60px",
              cursor: !isFormValid() || isSavingOrganization ? "not-allowed" : "pointer",
              whiteSpace: "nowrap",
            }}
            onClick={() => {
            ...
            <div className="layered-card-middle">
              ...
              <div className="layered-card-content">
                ...
                >
                      ...
                      >
                        ...
                        >
                          <i
                            ...
                            style={{ fontSize: 32, marginRight: 8, cursor: "pointer" }}
                          ></i>
                        </div>
                      </div>
                  ...
                  >
                    <span>
                      <i className="la la-plus-circle" style={{ fontSize: 17, marginRight: 8 }}></i>
                      Add more lines
                    </span>
                  </button>
                </div>
      ...
      {isSavingOrganization && (
        <FullScreenLoadingAnimation
          title={formType === "add" ? "Saving organization..." : "Updating organization..."}
          subtext={`Please wait while we are ${formType === "add" ? "saving" : "updating"} the organization`}
        />
      )}
    </div>
  );
