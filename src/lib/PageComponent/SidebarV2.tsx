...
export default function SidebarV2(props: any) {
  const { activeLink, navItems, footerNavItems, superAdminNavItems, isAdmin = false } = props;
  const [showAddOrgModal, setShowAddOrgModal] = useState(false);

  return (
    <>
      {showAddOrgModal && <AddOrgModal onClose={() => setShowAddOrgModal(false)} />}
        ...
        <div className="sidebar-header">
          <div>
            ...
            <nav className="nav-section">
              ...
              {navItems.map((item, idx) => (
                <Link href={item.href} key={item.label}>
                  <div className={`nav-item ${activeLink === item.label ? "active" : ""}`}>
                    <span>
                      <i className={item.icon} style={{ color: "#414651", fontSize: 24 }}></i>
                    </span>
            ...
            <div>
              ...
              >
                <span style={{ fontSize: 12, fontWeight: 500, color: "#717680" }}>© 2025</span>
