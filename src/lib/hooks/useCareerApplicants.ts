...
export function useCareerApplicants(initialStages: TimelineStages) {
  ...
  const interviewsInProgress = useMemo(
    () =>
      Object.values(timelineStages).reduce(
        (acc, stage) => acc + (stage.candidates?.length || 0),
        0
      ),
    [timelineStages]
  );
  const dropped = useMemo(
    () =>
      Object.values(timelineStages).reduce(
        (acc, stage) => acc + (stage.droppedCandidates?.length || 0),
        0
      ),
    [timelineStages]
  );
  const hired = useMemo(() => timelineStages["Hired"]?.candidates?.length || 0, [timelineStages]);
    ...
    for (const [key, stage] of Object.entries(newStages)) {
      sortedStages[key] = {
        ...
        candidates: stage.candidates.sort((a: any, b: any) => {
          const ratingA =
            (a.currentStep === "CV Screening" || key === "Pending AI Interview"
              ? a.cvStatus
              : a.jobFit) || "N/A";
          const ratingB =
            (b.currentStep === "CV Screening" || key === "Pending AI Interview"
              ? b.cvStatus
              : b.jobFit) || "N/A";
