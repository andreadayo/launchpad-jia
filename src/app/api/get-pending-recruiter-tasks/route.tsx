  ...
  try {
    ...
    return NextResponse.json({
      cvReview: pendingTasks.filter(
        (x) => x.currentStep === "CV Screening" && x.status !== "For AI Interview"
      ),
      aiInterviewReview: pendingTasks.filter(
        (x) => x.status !== "For Interview" && (x.currentStep === "AI Interview" || !x.currentStep)
      ),
      retakeRequest: pendingTasks.filter(
        (x) => x["retakeRequest"] && !["Approved", "Rejected"].includes(x["retakeRequest"].status)
      ),
    });
  } catch (error) {
    console.error(error);
